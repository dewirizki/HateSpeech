...
def report_results(model, X, y):
    ...
    f1 = f1_score(y, pred,labels=["H"],average='micro',pos_label="H")
    prec = precision_score(y, pred, labels=["H"],average='micro',pos_label="H")
    rec = recall_score(y, pred,  labels=["H"],average='micro',pos_label="H")
   
    return {'auc': auC, 'f1': f1, 'acc': acc, 'precision': prec, 'recall': rec}

def get_roc_curve(model, X, y):
    pred_proba = model.predict_proba(X)[:, 1]
    fpr, tpr, _ = roc_curve(y, pred_proba, pos_label='N')
    return fpr, tpr
